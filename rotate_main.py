...
class RotatorUI(ABC):
    @abstractmethod
    def display_rotations(self, rotations: dict):
        """Abstract method to display rotation results."""
        pass

class CLIRotatorUI(RotatorUI):
    def display_rotations(self, rotations: dict):
        print("\nRotation Results:")
        for filepath, rotation in rotations.items():
            print(f"{filepath} rotated by {rotation} degrees")

class GUIRotatorUI(RotatorUI):
    ...
    def display_rotations(self, rotations: dict):
        ...
        for filepath, rotation in rotations.items():
            self.list_widget.addItem(f"{filepath} rotated by {rotation} degrees")
...
def cli(overwrite_files: bool = False):
    ...
    ui = CLIRotatorUI()
    ui.display_rotations(rotations)
